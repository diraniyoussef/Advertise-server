...
class ServerSocketListener extends Thread {	//the main purpose is a socket listener
	...
	boolean createListenerSocket() {
		try {	    		    
			...
			* of ServerSocket 
			*/
			//server_socket  = new ServerSocket(port, 100, InetAddress.getByName("dirani.jvmhost.net"));
			//server_socket  = new ServerSocket(port, 100, InetAddress.getByName("74.122.199.173"));
			server_socket = new ServerSocket( port, 100, InetAddress.getByName("192.168.0.21") );
			server_socket.setSoTimeout( 2500 );
